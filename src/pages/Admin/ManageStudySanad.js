...
function ManageStudySanad() {
  ...
  async function uploadToStorage(file, folder) {
    ...
    try {
      ...
      // استخدام bucket 'asaneed' الموجود
  ...
  async function handleSubmit(e) {
    ...
    if (formData.file) {
      fileUrl = await uploadToStorage(formData.file, 'study_files');
    }
    if (formData.image) {
      imageUrl = await uploadToStorage(formData.image, 'study_images');
    }
    ...
    <div className="manage-study-page">
      <div className="container">
        <div className="page-header">
          ...
          <button className="btn-add" onClick={() => openModal()} disabled={uploading}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-submit" disabled={loading || uploading}>
                  {loading || uploading ? 'جاري الحفظ...' : <><FaSave /> {editingItem ? 'تحديث' : 'حفظ'}</>}
                </button>
              </div>
            </form>
          </div>
